...
def generate_sine_wave(maxValue=MAX_VAL):
    ...
    sine_wave_scaled = ((sine_wave + 1) / 2) * (maxValue - MIN_VAL) + MIN_VAL
...
class MsgPdFullPacket:
    def __init__(self, msg_type, amplitude):
        ...
        self.sine_wave = generate_sine_wave(amplitude).byteswap().tobytes()
...
class ServerThread(QThread):
    ...
    def run(self):
        ...
        self.pd_packet_start = MsgPdFullPacket(0x11, MAX_VAL)
        self.pd_packet_loop = MsgPdFullPacket(0x03, MAX_VAL)
        ...
        self.WaveAmplitudeUpDownflag = GOING_DOWN
        self.Amplitude = MAX_VAL
                        ...
                        if data:
                            ...
                            while 1:
                                ...
                                if packet_send_counter >= 60:
                                    print("60 Cycle Sent")
                                    packet_send_counter = 0
                                    if self.Amplitude <= (MIN_VAL + 400):
                                        self.WaveAmplitudeUpDownflag = GOING_UP
                                    elif self.Amplitude >= MAX_VAL:
                                        self.WaveAmplitudeUpDownflag = GOING_DOWN

                                    if self.WaveAmplitudeUpDownflag == GOING_UP:
                                        self.Amplitude += 200
                                        self.pd_packet_loop = MsgPdFullPacket(0x03, self.Amplitude)
                                        self.packet_loop = self.pd_packet_loop.fillup_data_packet()
                                    elif self.WaveAmplitudeUpDownflag == GOING_DOWN:
                                        self.Amplitude -= 200
                                        self.pd_packet_loop = MsgPdFullPacket(0x03, self.Amplitude)
                                        self.packet_loop = self.pd_packet_loop.fillup_data_packet()
